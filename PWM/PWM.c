...
void pwm_init(pwm_var *pwm_var) {
    ...
    SWM0->PINASSIGN.PINASSIGN7 = (SWM0->PINASSIGN.PINASSIGN7 & 0x00FFFFFF) 
                                | (pwm_var->pwm_ch0 << 24); // SCT0
    SWM0->PINASSIGN.PINASSIGN8 = (SWM0->PINASSIGN.PINASSIGN8 & 0x00000000) 
                                | (pwm_var->pwm_ch1 << 0)   // SCT1 
                                | (pwm_var->pwm_ch2 << 8)   // SCT2
                                | (pwm_var->pwm_ch3 << 16)  // SCT3
                                | (pwm_var->pwm_ch4 << 24); // SCT4 
    SWM0->PINASSIGN.PINASSIGN9 = (SWM0->PINASSIGN.PINASSIGN9 & 0xFFFFFF00) 
                                | (pwm_var->pwm_ch5 << 0); // SCT5
...
void set_pwm_duty(pwm_var *pwm_var, uint32_t pwm_duty[6]){
    ...
    SCT0->MATCHREL[2] = (pwm_var->period * pwm_duty[1]/100);
    SCT0->MATCHREL[3] = (pwm_var->period * pwm_duty[2]/100);
    SCT0->MATCHREL[4] = (pwm_var->period * pwm_duty[3]/100);
    SCT0->MATCHREL[5] = (pwm_var->period * pwm_duty[4]/100);
    SCT0->MATCHREL[6] = (pwm_var->period * pwm_duty[5]/100);
}
...
int main(void) {
    ...
    uint32_t pwm_d[6];  // Start at 0% duty
    pwm_d[0] = 10;  // 10-59%
    pwm_d[1] = 20;  // 20-69%
    pwm_d[2] = 33;  // 30-79%
    pwm_d[3] = 40;  // 40-89%
